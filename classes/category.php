...
class Category{
    ...
    public function viewOneCategory(){
        ...
        $this->CategoryQuantity_fld = $row['CategoryQuantity_fld'];
        }

    public function addCategory(){
            $sql = "INSERT INTO category_tbl SET CategoryId_fld=?, CategoryName_fld=?, CategoryQuantity_fld=?";

            $stmt = $this->conn_cv->prepare($sql);

            $stmt->bindparam(1,$this->CategoryId_fld);
            $stmt->bindparam(2,$this->CategoryName_fld);
            $stmt->bindparam(3,$this->CategoryQuantity_fld);
    ...
    public function updateCategory(){
            $sql = "UPDATE category_tbl SET CategoryName_fld=?, CategoryQuantity_fld=? WHERE CategoryId_fld=?";
    
            $stmt = $this->conn_cv->prepare($sql);
            
            $stmt->bindparam(1,$this->CategoryName_fld);
            $stmt->bindparam(3,$this->CategoryId_fld);
            $stmt->bindparam(2,$this->CategoryQuantity_fld);
